...
std::vector<exp_ass2::ErlOracle> oracle_msgs;
...
bool oracleService(exp_ass2::Oracle::Request &req, exp_ass2::Oracle::Response &res)
	{
		res.ID = winID;
		return true;
	}
...
void oracleCallback(const gazebo_msgs::LinkStates::ConstPtr& msg)
{
   for(int i=0; i< msg->name.size(); i++){
	   if (msg->name[i].find("cluedo_link")!= std::string::npos){
		   ...
		   for(int j=0; j<4;j++){
		   		//std::cout << "MARKER:" << j << "\n";
		   		double dist = distfromtarget(msg->pose[i].position.x, msg->pose[i].position.y, msg->pose[i].position.z, markx[j],marky[j],markz[j]);
		   		//std::cout << "X: "<< msg->pose[i].position.x;
		   		//std::cout << "Y: "<< msg->pose[i].position.y;
		   		//std::cout << "Z: "<< msg->pose[i].position.z;
		   		//std::cout << "DISTANZA LINK-MARKER: \n"; 
		   		//std::cout << dist <<'\n';
		   		//std::cout << "\n";
				if ((dist < 0.25 && ((lastmarkx !=markx[j]) || (lastmarky != marky[j])))){
  				exp_ass2::ErlOracle oracle_msg;
...
void provaclbk(const exp_ass2::NewHint::ConstPtr& msg){
	std::string oracle = msg->hint;
	ROS_INFO("ORACLE: [%s]", oracle.c_str());
}
...
oracle_pub = nh.advertise<exp_ass2::ErlOracle>( "/oracle_hint", 0 );
...
ros::Subscriber prova = nh.subscribe("/new_hint",10,provaclbk);
...
//std::cout<<marker.pose.position.z;
