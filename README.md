...
- the way the plan is designed, after the robot has visited all the waypoints initially, in subsequent searches only one waypoint at a time (randomly chosen) is changed to unvisited, as a result the robot every time it finds a new hint is forced to go back to the center of the arena and check if a hypothesis is ready, this wastes a lot of time. As another logic I had implemented a behavior of visiting all four waypoints at each round, this technique however produced a problem, in fact if the robot at the first waypoint completed the correct hypothesis, it still continued the round and it happened that for example at the third waypoint it completed a new hypothesis which was then 'overwritten' to the previous one. This sometimes resulted in the failure of the project as the correct hypothesis could be overwritten which would then no longer occur
